...
//main
let WarrantyMaterialCost = React.createClass({
  getInitialState() {
    return {
      visible: false,
      loading: true,
  ...
  showBudgetSummaryTables() {
    ...
    getBudgetSummaryTables(projectId).then((res) => {
      ...
      if(datas.MaterialCostQuality.contractArr) {

        tempContractArr = datas.MaterialCostQuality.contractArr.split(',');
      ...
      if(datas.MaterialCostQuality.qualityArr) {

        tempQualityArr = datas.MaterialCostQuality.qualityArr.split(',');
  ...
  handleSubmit() {
    ...
    _self.props.form.validateFieldsAndScroll((errors, formDatas) => {
      ...
      //检查materialCostQualitiesId是否存在
      let materialCostQualitiesId = _self.state.budgetSummaryDatas.MaterialCostQuality.id || 0;
      if(!materialCostQualitiesId) return message.warning("获取质保期材料费信息失败！");
      ...
      updateMaterialCostQualities(projectId, materialCostQualitiesId, JSON.stringify(datas)).then((res) => {
...
WarrantyMaterialCost = Form.create()(WarrantyMaterialCost);

export default WarrantyMaterialCost;