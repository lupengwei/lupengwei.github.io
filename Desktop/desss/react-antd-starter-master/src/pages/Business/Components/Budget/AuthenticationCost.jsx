...
//main
let AuthenticationCost = React.createClass({
  getInitialState() {
    return {
      visible: false,
      loading: true,
  ...
  showBudgetSummaryTables() {
    ...
    getBudgetSummaryTables(projectId).then((res) => {
      ...
      if(datas.CertificationCost.contractArr) {

        tempContractArr = datas.CertificationCost.contractArr.split(',');
      ...
      if(datas.CertificationCost.qualityArr) {

        tempQualityArr = datas.CertificationCost.qualityArr.split(',');
  ...
  handleSubmit() {
    ...
    _self.props.form.validateFieldsAndScroll((errors, formDatas) => {
      ...
      //检查certificationCostId是否存在
      let certificationCostId = _self.state.budgetSummaryDatas.CertificationCost.id || 0;
      if(!certificationCostId) return message.warning("获取质保期材料费信息失败！");
      ...
      updateCertificationCosts(projectId, certificationCostId, JSON.stringify(datas)).then((res) => {

        if(res.jsonResult.code < 0) return message.warning(res.jsonResult.msg);

        message.success("费用信息添加成功！");
        _self.setState({ visible: false, loading: true });
...
AuthenticationCost = Form.create()(AuthenticationCost);

export default AuthenticationCost;