...
//main
let BudgetPaymentPlan = React.createClass({
  getInitialState() {
    return {
      visible: false,
      loading: true,
  ...
  showBudgetSummaryTables() {
    ...
    getBudgetSummaryTables(projectId).then((res) => {
      ...
      if(datas.BudgetPaymentPlan.contractArr) {

        tempContractArr = datas.BudgetPaymentPlan.contractArr.split(',');
      ...
      if(datas.BudgetPaymentPlan.qualityArr) {
        
        tempQualityArr = datas.BudgetPaymentPlan.qualityArr.split(',');
  ...
  handleSubmit() {
    ...
    _self.props.form.validateFieldsAndScroll((errors, formDatas) => {
      ...
      //检查budgetPaymentPlanId是否存在
      let budgetPaymentPlanId = _self.state.budgetSummaryDatas.BudgetPaymentPlan.id || 0;
      if(!budgetPaymentPlanId) return message.warning("获取回款预算信息失败！");
      ...
      updateBudgetPaymentPlan(projectId, budgetPaymentPlanId, JSON.stringify(datas)).then((res) => {
...
BudgetPaymentPlan = Form.create()(BudgetPaymentPlan);

export default BudgetPaymentPlan;