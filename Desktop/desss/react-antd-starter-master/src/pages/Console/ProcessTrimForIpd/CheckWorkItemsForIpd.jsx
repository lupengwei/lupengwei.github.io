  ...
  handleSubmit(params) {
    ...
    _self.props.form.validateFieldsAndScroll((errors, formDatas) => {
      ...
      if(params == 'accept') {
        ...
        datas = {
          option: 'accept',
          qpSuggestion: formDatas.qpSuggestion
        };

      }else if(params == 'refuse') {

        if(!formDatas.qpSuggestion) return message.warning("请填写拒绝理由！");
      ...
      checkStageTrimForIpd(projectId, JSON.stringify(datas)).then((res) => {
        ...
        if(params == 'refuse') {
          ...
          setTimeout(function(){
            _self.context.router.push('/console/projects/editTasksForIpd');
          }, 500)
        }
            ...
            :
              <Form horizontal form={props.form}>
                <FormItem
                  ...
                  {...formItemLayout}
                >
                  <Input type="textarea" rows={4} {...getFieldProps('qpSuggestion', {rules: [{message: '请输入审核意见' }]})} placeholder="请输入审核意见（默认为同意）" />
                </FormItem>
