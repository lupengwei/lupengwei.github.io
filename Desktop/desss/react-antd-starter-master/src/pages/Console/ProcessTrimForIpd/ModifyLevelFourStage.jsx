...
//main
let ModifyLevelFourStage = React.createClass({
  getInitialState() {
    return {
      loading: true,
  ...
  handleModifySubmit() {
    let _self = this;
    let levelFourStageId = _self.state.taskDatas.id;
    ...
    _self.props.form.validateFieldsAndScroll((errors, formDatas) => {
      ...
      if(_self.state.isSelected) {
        ...
        if(!formDatas.fzr) return message.warning('请完善已启用的任务信息！');
      ...
      updateLevelFourStageForIpd(levelFourStageId, JSON.stringify(datas)).then((res) => {
    ...
    return (
      <Spin spinning={state.loading}>
        <div className="desp-modal-task-title">
          ...
          <span>{state.taskDatas.name}<font style={{color:'#CCC',fontSize:12,fontWeight:400}}> . 四级工作项</font></span>
        </div>
...
ModifyLevelFourStage = Form.create()(ModifyLevelFourStage);

export default ModifyLevelFourStage;