...
//main
let LogisticCost = React.createClass({
  getInitialState() {
    return {
      visible: false,
      loading: true,
      ...
      initDatas: ['车载设备包装费', '轨旁设备包装费', '室内设备包装费', '车载设备运输费', '轨旁设备运输费', '室内设备运输费', '运输保险费'], //初始化数据
    };
  },
  componentDidMount() {
  ...
  deleteCostItem(value) {
    ...
    confirm({
      ...
      onOk() {
        deleteTransportCosts(projectId, value.recordId).then(() => {
  ...
  handleSubmit() {
    ...
    _self.props.form.validateFieldsAndScroll((errors, formDatas) => {
      ...
      createTransportCosts(projectId, JSON.stringify(datas)).then(() => {
...
LogisticCost = Form.create()(LogisticCost);

export default LogisticCost;