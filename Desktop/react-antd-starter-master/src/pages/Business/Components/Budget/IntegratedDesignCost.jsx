...
//main
let IntegratedDesignCost = React.createClass({
  getInitialState() {
    return {
      visible: false,
      loading: true,
  ...
  showBudgetSummaryTables() {
    ...
    getBudgetSummaryTables(projectId).then((res) => {
      ...
      if(datas.DesignCost.contractArr) {

        tempContractArr = datas.DesignCost.contractArr.split(',');
      ...
      if(datas.DesignCost.qualityArr) {

        tempQualityArr = datas.DesignCost.qualityArr.split(',');
  ...
  handleSubmit() {
    ...
    _self.props.form.validateFieldsAndScroll((errors, formDatas) => {
      ...
      //检查designCostId是否存在
      let designCostId = _self.state.budgetSummaryDatas.DesignCost.id || 0;
      if(!designCostId) return message.warning("获取质集成设计费信息失败！");
      ...
      updateDesignCosts(projectId, designCostId, JSON.stringify(datas)).then((res) => {
...
IntegratedDesignCost = Form.create()(IntegratedDesignCost);

export default IntegratedDesignCost;