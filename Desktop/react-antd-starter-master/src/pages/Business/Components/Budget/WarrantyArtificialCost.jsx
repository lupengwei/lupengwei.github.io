...
//main
let WarrantyArtificialCost = React.createClass({
  getInitialState() {
    return {
      visible: false,
      loading: true,
  ...
  showBudgetSummaryTables() {
    ...
    getBudgetSummaryTables(projectId).then((res) => {
      ...
      if(datas.LabourCostQuality.contractArr) {

        tempContractArr = datas.LabourCostQuality.contractArr.split(',');
      ...
      if(datas.LabourCostQuality.qualityArr) {

        tempQualityArr = datas.LabourCostQuality.qualityArr.split(',');
  ...
  handleSubmit() {
    ...
    _self.props.form.validateFieldsAndScroll((errors, formDatas) => {
      ...
      //检查labourCostQualityId是否存在
      let labourCostQualityId = _self.state.budgetSummaryDatas.LabourCostQuality.id || 0;
      if(!labourCostQualityId) return message.warning("获取质保期人工费信息失败！");
      ...
      updateLabourCostQualities(projectId, labourCostQualityId, JSON.stringify(datas)).then((res) => {
...
WarrantyArtificialCost = Form.create()(WarrantyArtificialCost);

export default WarrantyArtificialCost;